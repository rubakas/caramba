...
# yet tracked (no matching Show/Movie record, no existing PendingImport),
# and enqueues them as PendingImports with candidate matches from
# TvmazeService (shows) or ImdbApiService (movies).
#
...
class LibraryWatcherService
  ...
  class << self
    ...
    def already_known?(kind, entry_path)
      case kind
      when "shows"
        return true if Show.exists?(media_path: entry_path)
      when "movies"
    ...
    def build_pending_import(media_folder, entry_path)
      case media_folder.kind
      when "shows"
        build_show_import(media_folder, entry_path)
      when "movies"
