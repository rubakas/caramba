...
```bash
bin/build              # Build for macOS
bin/build --publish    # Build + publish to GitHub Releases
```
...
Output goes to `dist/` — look for `.dmg` files.
...
- Detects `.dmg` asset from the release
...
- Mounts DMG, stages the `.app` to a temp dir, spawns a detached shell script that waits for the app to quit, copies the new `.app` to `/Applications/`, and relaunches
...
vendor/                Bundled binaries (gitignored, created by bin/setup-ffmpeg)
  ffmpeg/              macOS static ffmpeg + ffprobe (arm64 + x64)
